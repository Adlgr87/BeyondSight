...
# ── BeyondSight offline forecast (deterministic proxy) ───────────────────────

def _beyondsight_offline_forecast(
    train: np.ndarray,
    horizon: int,
    rng: np.random.Generator,
) -> np.ndarray:
    """Deterministic proxy for BeyondSight in offline (no-LLM) mode.
    ...
    This is *not* the real BeyondSight prediction; it exists solely so CI
...
def _beyondsight_llm_forecast(
    train: np.ndarray,
    horizon: int,
    case: dict,
) -> np.ndarray | None:
    """Attempt a real BeyondSight LLM-backed forecast.
...
def evaluate_case(
    ...
    """Evaluate all baselines (and optionally BeyondSight) on one case.
    ...
    # ── BeyondSight forecast ──────────────────────────────────────────────────
    if mode == "llm":
        bs_pred = _beyondsight_llm_forecast(train, horizon, case)
    else:
        bs_pred = _beyondsight_offline_forecast(train, horizon, rng)
...
def generate_report(results: list[dict], mode: str, seed: int) -> str:
    ...
    lines = [
        ...
        "> (see `docs/validation/PVU_BeyondSight_EN.md`).",
        "",
        "---",
        "",
    ]
    ...
    for r in results:
        ...
        bs = r.get("beyondsight", {})
        if bs:
            lines += [
                "",
                "### BeyondSight metrics (test split)",
                "",
    ...
    lines += [
        "---",
        "",
        "_Generated by `benchmarks.runner` — BeyondSight PVU-BS_",
    ]
    return "\n".join(lines)
...
def _parse_args(argv=None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        ...
        description="BeyondSight PVU-BS offline benchmark runner.",
    )
    parser.add_argument(
        "--cases",
    ...
    mode_group.add_argument(
        ...
        action="store_true",
        default=False,
        help="Run BeyondSight in LLM mode (requires API key).",
    )
    parser.add_argument(
        "--out",
